    ...
    createfunctionyml: function (fun) {
        ...
        for (f of pro.functions) {
            if (f.inputs.name == fun) {
                let str = "TENCENT_SECRET_ID=" + pro.id + "\nTENCENT_SECRET_KEY=" + pro.key
                fs.writeFile('.env', str, "utf-8", function (err) {
                    if (err) {
                        console.error(err)
                        dialog.showErrorBox("失败", "生成授权文件失败:" + JSON.stringify(err))
                        return false
                    };
                    return true
                })
    ...
    saveConfigFun(yml) {
        ...
        for (let i = 0; i < set.functions.length; ++i) {
            if (set.functions[i].name == yml.name) {
                let dlg = dialog.showMessageBoxSync({
                    type: "warning",
                    buttons: ["覆盖", "取消"],
                    defaultId: 0,
                    message: "该函数已经在配置文件存在,您是否要覆盖函数",
